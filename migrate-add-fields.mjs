...
const STMT = (t) => `ALTER TABLE \`${t}\`
  ...
  ADD COLUMN ops_advertiser_name VARCHAR(255) NOT NULL DEFAULT '' AFTER advertiser,
  ADD COLUMN ai_evaluate_reviewer_name VARCHAR(128) NOT NULL DEFAULT '' AFTER ops_advertiser_name`
...
try {
  for (const t of TABLES) {
    ...
    if (need.includes('ops_advertiser_name')) cols.push(`ADD COLUMN ops_advertiser_name VARCHAR(255) NOT NULL DEFAULT '' AFTER advertiser`)
    if (need.includes('ai_evaluate_reviewer_name')) cols.push(`ADD COLUMN ai_evaluate_reviewer_name VARCHAR(128) NOT NULL DEFAULT '' AFTER ops_advertiser_name`)
